...
//import HeartbeatView from 'sections/apps/customer/HeartbeatView';
...
function ReactTable({ columns, data, handleAdd }: Props) {
  ...
  const sortBy = { id: 'syncStatus', desc: false };
...
const HeartbeatCardPage = () => {
  ...
  const columns = useMemo(
    () => [
      ...
      // },
      {
        ...
        disableSortBy: true,
        Cell: ({ row }: { row: Row<{}> }) => {
          // const collapseIcon = row.isExpanded ? <Add style={{ color: theme.palette.error.main, transform: 'rotate(45deg)' }} />: <div></div>;
          return (
  ...
  //const renderRowSubComponent = useCallback(({ row }: { row: Row<{}> }) => <HeartbeatView data={data[Number(row.id)]} />, [data]);

  return (
    <MainCard content={false}>
      <ScrollX>
        <ReactTable columns={columns} data={data} handleAdd={handleAdd} />
      </ScrollX>
...
export default HeartbeatCardPage;
